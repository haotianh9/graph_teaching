...
def main() -> None:
    parser = argparse.ArgumentParser(description="Mux Chinese TTS onto Video 5.3 scenes.")
    parser.add_argument("--video-dir", default="media/videos/main/720p30")
    parser.add_argument("--audio-dir", default="assets/audio/zh")
    parser.add_argument("--scene-out-dir", default="media/videos/main/720p30_zh_tts")
    parser.add_argument("--final-out", default="media/videos/video5_3_zh_tts_review_720p30.mp4")
    ...
    for scene in SCENE_ORDER:
        ...
        filter_spec = (
            f"[0:v]tpad=stop_mode=clone:stop_duration={max(0.0, audio_duration - video_duration):.3f},"
            f"trim=0:{output_duration:.3f},setpts=PTS-STARTPTS[v];"
            f"[1:a]apad=pad_dur={max(0.0, video_duration - audio_duration):.3f},"
        ...
        print(f"{scene}: video={video_duration:.1f}s audio={audio_duration:.1f}s output={output_duration:.1f}s")

    concat_file = scene_out_dir / "concat_zh_tts.txt"
    concat_file.write_text(
        "".join(f"file '{path.resolve()}'\n" for path in muxed_paths),
        encoding="utf-8",
    )
    run(["ffmpeg", "-y", "-v", "error", "-f", "concat", "-safe", "0", "-i", str(concat_file), "-c", "copy", str(final_out)])
